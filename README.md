...
- **이미지 첨부:** 채팅 목록에서는 **카드 리스트**로 표시하고, **뷰어로 열기**·라이트박스에서는 서버 **미리보기**(`GET .../preview`) 우선·원본 스트림; 업로드 시 클라이언트 압축본을 `preview`로 보내면 DB·메시지에 `previewSizeBytes` 반영. 다운로드는 **원본** vs **미리보기(압축)** 선택 시 각각 저장 용량 표시, 구버전 첨부(미리보기 없음)는 **약 512KB 이상**일 때만 원본 vs 브라우저 JPEG 재인코딩(`modalImageDownloadChoice`, GIF/SVG는 원본만). 클립보드 **붙여넣기(Ctrl+V)** 로 이미지 **여러 장** 동시 첨부 가능
- **첨부 UX:** 채팅 영역 **드래그 앤 드롭**·파일 선택 **다중(`multiple`)**·붙여넣기로 **여러 파일**을 한 번에 올린 뒤 순차 전송; 업로드 즉시 채팅 본문에 **카드 리스트**(아이콘·파일명·크기 + 저장/저장 후 열기/뷰어로 열기)로 표시 + 햄버거 메뉴 **첨부파일**·**이미지 모아보기**(채널/DM 공통; 전체 탭은 비이미지 첨부만, 이미지는 이미지 탭 전용). **같은 분에 연속으로 올린 FILE 메시지**(이미지·문서 혼합 가능)는 **한 묶음**으로 보이며 **일괄저장**으로 원본을 순차 저장할 수 있음
...
- **채팅 UX:** 날짜 변경 시 구분선(pill), 파일·이미지 첨부는 **카드 리스트**로 표시(뷰어는 이미지는 라이트박스), DM **사이드바**·**채팅 헤더**에 상대 **프레즌스 점**, 좌측 하단 본인 상태는 실시간 프레즌스와 동기; 입력은 **Enter 전송·Shift+Enter 줄바꿈**, 본문 줄바꿈 표시, **본인 메시지**는 오른쪽 정렬·아바타 숨김, **채널별** 작성 중(미전송 텍스트·답글·대기 첨부) 저장; 미읽음 **「새 메시지」** 라인은 입력·전송 시 사라짐
