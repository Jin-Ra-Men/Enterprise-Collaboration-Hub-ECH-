...
- **채널 운영 확장:** 우측 상단 **햄버거 메뉴**에서 업무/첨부/구성원/나가기/채널 폐쇄를 통합 제공, 관리자는 채널명 변경 가능, 관리자 위임은 멤버 우클릭 메뉴로 수행
...
- **채널 업무 허브:** 채널 헤더 `📋`에서 업무·칸반을 한 모달에서 조회하고, 신규/상태/컬럼 이동/순서 변경은 **저장**으로 반영, 항목 **삭제(✕)**, 칸반 **채널 멤버**(본인 포함) 담당 지정·자동완성(이름/조직·직급 구분, 키보드 선택 지원), 카드 순서는 세로 **드래그앤드롭**으로 조정
- **내 담당 칸반 바로가기:** 좌측 사이드바(채널/DM 목록 영역)에 `내 담당 칸반` 섹션을 제공해 담당 카드 목록을 확인하고 클릭 시 해당 채널의 업무·칸반 모달로 즉시 이동
...
- **테마:** 로그인 사용자 라인(로그아웃 옆 톱니바퀴)에서 팝업으로 검정/하양/파랑 선택, 사용자별 설정을 DB(`users.theme_preference`)에 저장해 로그아웃 후 재로그인에도 유지
