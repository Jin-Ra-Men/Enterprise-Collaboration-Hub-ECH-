...
- **채널 업무 허브:** 채널 헤더 `📋`에서 업무·칸반을 한 모달에서 조회하고, 신규/상태/컬럼 이동/순서 변경은 **저장**으로 반영, 항목 **삭제(✕)**, 칸반 **채널 멤버**(본인 포함) 담당 지정·자동완성(이름/조직·직급 구분, 키보드 선택 지원), 카드 순서는 세로 **드래그앤드롭**으로 조정. **저장** 성공 시 사이드바 **내 업무 항목** 목록도 즉시 갱신
