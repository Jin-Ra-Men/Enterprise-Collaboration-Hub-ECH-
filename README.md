...
- **채널 업무 허브:** 채널 헤더 `📋`에서 업무·칸반을 한 모달에서 조회하고, 신규/상태/컬럼 이동/순서 변경은 **저장**으로 반영, 항목 **삭제(✕)**, 칸반 **채널 멤버**(본인 포함) 담당 지정·자동완성(이름/조직·직급 구분, 키보드 선택 지원), 칸반 카드는 제목 왼쪽 **⋮⋮ 드래그 핸들**(넉넉한 탭/클릭 영역)을 잡아 컬럼 안·**컬럼 간** 이동(Chrome 등에서 카드 전체 `draggable` 시 하단 컬럼 `<select>` 표시가 어긋나는 문제 방지)하며 저장 시 컬럼에 맞게 카드 `status`도 함께 반영. **저장** 성공 시 사이드바 **내 업무 항목** 목록도 즉시 갱신
