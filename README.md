...
- **채널 운영 확장:** 우측 상단 **햄버거 메뉴**에서 업무/첨부/구성원/**알림 끄기·켜기**(다른 채의 **일반 메시지 토스트만** 끔; **멘션 토스트·미읽음 배지는 유지**)/나가기/채널 폐쇄를 통합 제공, 관리자는 채널명 변경 가능, 관리자 위임은 멤버 우클릭 메뉴로 수행. 채널·DM·퀵 레일 목록 **우클릭**으로도 동일하게 알림 토글·나가기 가능(설정은 브라우저 `localStorage` 채널 ID 목록)
...
- **첨부 UX:** 업로드 즉시 채팅 본문에 파일 카드 표시 + 햄버거 메뉴 **첨부파일**·**이미지 모아보기**(채널/DM 공통, 모달에서 전체 목록·이미지 그리드 탭)
