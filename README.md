...
- **읽음 포인터·미읽음 배지:** 채널별 마지막으로 읽은 메시지 저장·조회; 목록 API `unreadCount`(루트 메시지 기준)로 채널/DM 사이드바에 빨간 원형 숫자 배지; **`lastMessageAt`** 로 최근 대화 순; 좌측 **퀵 레일**(64px, 세로 아이콘·짧은 라벨·배지, 미읽음만); 사이드바 **접기**는 우측 가운데 **돌출 탭**·`localStorage` `ech_sidebar_collapsed`
