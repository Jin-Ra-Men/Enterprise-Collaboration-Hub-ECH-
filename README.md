...
- **읽음 포인터·미읽음 배지:** 채널별 마지막으로 읽은 메시지 저장·조회; 목록 API `unreadCount`(루트 메시지 기준)로 채널/DM 사이드바에 빨간 원형 숫자 배지; **`lastMessageAt`** 로 최근 대화 순; **퀵 레일**은 ECH 제목 아래·검색~목록과 같은 높이 구간에만(64px 열); **미읽음 우선·최근 대화** 최대 15개·배지는 미읽음만; 패널 펼침 **324px**; **접기**는 우측 **돌출 탭**·`localStorage` `ech_sidebar_collapsed`
...
### 통합 검색(Phase 3-6)
- 메시지/파일/업무 항목/칸반 카드를 단일 API로 검색
