...
- **이미지 첨부:** 인라인·썸네일은 서버 **미리보기**(`GET .../preview`), 확대는 **원본 스트림**; 업로드 시 클라이언트 압축본을 `preview`로 보내면 DB·메시지에 `previewSizeBytes` 반영. 다운로드는 **원본** vs **미리보기(압축)** 선택 시 각각 저장 용량 표시, 구버전 첨부(미리보기 없음)는 **약 512KB 이상**일 때만 원본 vs 브라우저 JPEG 재인코딩(`modalImageDownloadChoice`, GIF/SVG는 원본만). 클립보드 **붙여넣기(Ctrl+V)** 로 이미지 **여러 장** 동시 첨부 가능
- **첨부 UX:** 채팅 영역 **드래그 앤 드롭**·파일 선택 **다중(`multiple`)**·붙여넣기로 **여러 파일**을 한 번에 올린 뒤 순차 전송; 업로드 즉시 채팅 본문에 파일 카드 표시 + 햄버거 메뉴 **첨부파일**·**이미지 모아보기**(채널/DM 공통; 전체 탭은 비이미지 첨부만, 이미지는 이미지 탭 전용)
...
- **채팅 UX:** 날짜 변경 시 구분선(pill), 파일·이미지 업로드는 일반 메시지와 같은 행 레이아웃(이미지는 인라인 썸네일), DM **사이드바**·**채팅 헤더**에 상대 **프레즌스 점**, 좌측 하단 본인 상태는 실시간 프레즌스와 동기; 입력은 **Enter 전송·Shift+Enter 줄바꿈**, 본문 줄바꿈 표시, **본인 메시지**는 오른쪽 정렬·아바타 숨김, **채널별** 작성 중(미전송 텍스트·답글·대기 첨부) 저장; 미읽음 **「새 메시지」** 라인은 입력·전송 시 사라짐
