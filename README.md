...
- **채널 운영 확장:** 우측 상단 **햄버거 메뉴**에서 업무/첨부/구성원/**알림 끄기·켜기**/나가기/채널 폐쇄를 통합 제공, 관리자는 채널명 변경 가능, 관리자 위임은 멤버 우클릭 메뉴로 수행. 채널·DM·퀵 레일 목록 **우클릭**으로도 동일하게 알림 토글·나가기 가능(알림 음소거는 브라우저 `localStorage`에 채널 ID 목록 저장)
