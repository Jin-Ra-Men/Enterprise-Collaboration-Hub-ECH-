...
- **이미지 첨부:** 인라인·썸네일은 서버 **미리보기**(`GET .../preview`), 확대는 **원본 스트림**; 업로드 시 클라이언트 압축본을 `preview`로 보내면 DB·메시지에 `previewSizeBytes` 반영. 다운로드는 **원본** vs **미리보기(압축)** 선택 시 각각 저장 용량 표시, 구버전 첨부(미리보기 없음)는 **약 512KB 이상**일 때만 원본 vs 브라우저 JPEG 재인코딩(`modalImageDownloadChoice`, GIF/SVG는 원본만). 클립보드 **붙여넣기(Ctrl+V)** 지원
