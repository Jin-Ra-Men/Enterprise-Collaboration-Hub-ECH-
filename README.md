...
- **자동 업데이트**: 패키지 실행 시 `electron-updater`가 GitHub Releases(`build.publish`의 owner/repo)를 조회합니다. 릴리즈 에셋에 **`latest.yml`**과 설치 파일(및 가능하면 **`*.exe.blockmap`**)을 함께 올려야 합니다. `tools/publish-electron-github-release.ps1`로 일괄 업로드할 수 있습니다(`GITHUB_TOKEN` 필요). 새 버전 다운로드가 끝나면 **메인 창 모달**에서 안내 후 **확인** 시 설치·재시작합니다. **내부망**에서는 GitHub 대신 사내 백엔드 `/desktop-updates/` 를 쓰는 방식이며, 설명·절차는 `docs/DEPLOYMENT_WINDOWS.md` **「데스크톱 앱 자동 업데이트(내부망)」** 를 본다. GitHub 릴리즈에 **Source code (zip/tar)** 가 함께 보이는 것은 **GitHub가 태그마다 자동으로 붙이는 항목**이며, 위 스크립트가 소스 zip을 따로 올리는 것이 아닙니다.
