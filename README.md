...
> **배포 버전(데스크톱·백엔드 JAR):** **1.1.1** (Git 태그 `v1.1.1`). 변경 요약은 `.cursor/rules/CHANGELOG.md` 참고.
...
- **첨부 UX:** 채팅 영역 **드래그 앤 드롭**·파일 선택 **다중(`multiple`)**·붙여넣기로 **여러 파일**을 한 번에 올린 뒤 순차 전송; 비이미지는 **카드**(저장·저장 후 열기), 이미지는 **그리드**로 표시 + 햄버거 메뉴 **첨부파일**·**이미지 모아보기**(채널/DM 공통; 전체 탭은 비이미지 첨부만, 이미지는 이미지 탭 전용). **같은 분에 연속으로 올린 FILE 메시지**(이미지·문서 혼합 가능)는 **한 묶음**으로 보이며 **일괄저장**으로 원본을 순차 저장할 수 있음
- **칸반 보드:** 워크스페이스별 보드·컬럼·카드 CRUD, 담당자 지정, 상태/컬럼 이동 이력 API
- **채팅→업무:** 메시지 ID 기준 업무 항목 생성·조회(메시지와 1:1 링크)
- **채널 업무 허브:** 채널 헤더 `📋`에서 업무·칸반을 한 모달에서 조회하고, 신규/상태/컬럼 이동/순서 변경은 **저장**으로 반영, 항목 **삭제(✕)**, 비활성 업무 **복원·완전 삭제**는 목록·상세에서 예약 후 **저장** 시에만 서버 반영(예약 행은 배지로 표시), 칸반 **채널 멤버**(본인 포함) 담당 지정·자동완성(이름/조직·직급 구분, 키보드 선택 지원), 칸반 카드는 **제목·본문 등 카드 본체**를 잡아 컬럼 안·**컬럼 간** 드래그(검색창·컬럼 `<select>`·버튼 위에서는 드래그가 시작되지 않음)하며, 드롭 후 컬럼 `<select>`는 DOM 재생성으로 Chrome 표시 불일치를 완화한다. 저장 시 컬럼에 맞게 카드 `status`도 함께 반영. **저장** 성공 시 사이드바 **내 업무 항목** 목록도 즉시 갱신
...
- **첨부「저장 후 열기」**: 데스크톱에서는 다운로드와 함께 임시 파일을 저장한 뒤 `shell.openPath`로 **OS 기본 연결 앱**으로 연다(예: `.txt` → 메모장 등 사용자 연결). **브라우저만** 쓸 때는 기존처럼 blob **새 탭**으로 열린다.
...
- **자동 업데이트**: 패키지 실행 시 `electron-updater`가 GitHub Releases(`build.publish`의 owner/repo)를 조회합니다. 릴리즈 에셋에 **`latest.yml`**과 설치 파일(및 가능하면 **`*.exe.blockmap`**)을 함께 올려야 합니다. `tools/publish-electron-github-release.ps1`로 일괄 업로드할 수 있습니다(`GITHUB_TOKEN` 필요). 새 버전 다운로드가 끝나면 **메인 창 모달**에서 안내 후 **확인** 시 설치·재시작합니다. **내부망**에서는 GitHub 대신 사내 백엔드 `/desktop-updates/` 를 쓰는 방식이며, 설명·절차는 `docs/DEPLOYMENT_WINDOWS.md` **「데스크톱 앱 자동 업데이트(내부망)」** 를 본다. GitHub 릴리즈에 **Source code (zip/tar)** 가 함께 보이는 것은 **GitHub가 태그마다 자동으로 붙이는 항목**이며, 위 스크립트가 소스 zip을 따로 올리는 것이 아닙니다. 이미지 크게 보기: 서버에 미리보기가 있으면 먼저 압축 JPEG를 보여 주고 **원본 보기**로 전환할 수 있습니다(채팅 인라인·파일 허브·검색 공통).
