...
- **이미지 첨부:** 채팅에 인라인 미리보기, 클릭 시 확대(라이트박스), 확대 화면에서 다운로드; **약 512KB 이상** 이미지는 다운로드 시 **원본**과 **JPEG 압축본** 중 선택(`modalImageDownloadChoice`, GIF/SVG는 원본만). 채팅 화면에서 클립보드 이미지 **붙여넣기(Ctrl+V)** 로 전송 대기(기존 파일 첨부·전송 흐름과 동일)
