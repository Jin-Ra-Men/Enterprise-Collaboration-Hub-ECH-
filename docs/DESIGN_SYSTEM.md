...
|------|-----------------|-------------------------|
| `(1)` | ECH Workspace — Work Management | **워크플로우 페이지 뷰** `#modalWorkHub` · `.ech-workhub-shell` · `.work-hub-panel--workflow`(단일 패널·Tasks/Board 섹션) · `#workHubChannelContext` 채널·DM 연결 표시 |
...
| `(6)` | Task & Kanban Management | 워크플로우 페이지 `#modalWorkHub`의 칸반 컬럼 `#channelKanbanBoard` |
