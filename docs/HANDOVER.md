...
- **네트워크 단절 후 채팅 복구**: 채널 화면에 `메시지 로드 실패`가 표시된 뒤 연결이 돌아오면, 프론트 `recoverActiveChannelTimelineIfNeeded`가 `online`·소켓 `connect/reconnect` 훅에서 현재 채널 `loadMessages`를 자동 재시도한다(앱 재시작 불필요).
