...
- `GET /api/user-directory/organization` 응답은 `data.companies[]` → `divisions[]` → `teams[]` → `users[]` 3단계 트리이며, DB의 `company_name` / `division_name` / `team_name`(및 기존 `department` 기반 폴백 유추)을 사용합니다. 프론트는 채널·DM 생성 모달과 구성원 추가에서 모두 메인 모달의 **+** 버튼으로 `modalAddMemberPicker` 오버레이를 열고, 좌측 조직도 + 우측 검색/결과에서 선택한 사용자를 상위 모달의 선택 태그에 반영합니다.
