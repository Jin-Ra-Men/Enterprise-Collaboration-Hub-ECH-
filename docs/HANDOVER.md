...
- **개발자**
  ...
  - **AI 게이트웨이 Phase 7-1(스텁):** `docs/AI_GATEWAY_POLICY.md`. 엔드포인트 `GET /api/ai/gateway/status`, `POST /api/ai/gateway/chat`. 기본 `app.ai.allow-external-llm=false` 일 때 chat은 **403** (`AI_GATEWAY_BLOCKED`), 감사 `AI_GATEWAY_POLICY_BLOCKED`(프롬프트 본문 미저장). 플래그 허용 시에도 제공자 미연결로 **501** 스텁 (`AI_GATEWAY_PROVIDER_NOT_CONFIGURED`). 운영에서 허용 전 법무·보안 절차 필수.
