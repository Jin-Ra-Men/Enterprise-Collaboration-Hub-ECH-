...
- **개발자**
  ...
  - **AI 게이트웨이 Phase 7-1(스텁):** `docs/AI_GATEWAY_POLICY.md`. 엔드포인트 `GET /api/ai/gateway/status`, `POST /api/ai/gateway/chat`. 기본 `app.ai.allow-external-llm=false` 일 때 chat은 **403** (`AI_GATEWAY_BLOCKED`), 감사 `AI_GATEWAY_POLICY_BLOCKED`(프롬프트 본문 미저장; `citedDistinct`, `piiRedactions` 등 메타). **`citedMessageIds`가 있으면 `channelId` 필수**, 각 ID는 해당 채널 메시지이며 호출자는 멤버여야 한다. `AiGatewayPiiMasker`가 게이트웨이 내 휴리스틱 마스킹(완전하지 않음). 플래그 허용 시에도 제공자 미연결로 **501** 스텁 (`AI_GATEWAY_PROVIDER_NOT_CONFIGURED`). 운영에서 허용 전 법무·보안 절차 필수.
