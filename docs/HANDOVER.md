...
- 프론트 `loadChannelKanbanBoard()`는 DnD/저장/셀렉트 등으로 중첩 호출될 수 있다. **`kanbanBoardFetchGenByChannelId`**로 채널별 요청 세대를 올리고, 완료 시점에 세대·현재 허브 채널이 맞을 때만 `renderKanbanBoard`를 호출해, 늦게 도착한 **이전 GET 응답**이 UI를 덮어쓰지 않게 한다.
