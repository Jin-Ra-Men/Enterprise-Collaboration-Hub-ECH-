...
- **개발자**
  ...
  - **AI 프로액티브·제안함 (Phase 7-3 MVP + 후속):** 채널 옵트인·DM 차단·사용자 톤/다이제스트·**개인 `ai_assistant_enabled` 마스터 스위치**(테마 설정·`/api/me` 반영)·제안함 큐·거절 쿨다운·채널 시간당 적재 상한·적재 시 옵트인·수신자 마스터 검증. 스케줄러 `ProactiveAiSuggestionScheduler`: 활동 힌트(매시 `:07`), 다이제스트 배치(매일 09:15 `Asia/Seoul`). **LLM 대화 인사이트** `ProactiveConversationInsightScheduler`(매시 `:23`): 기본 비활성 시드 `ai.proactive.llm-conversation-insight-enabled=false`; 외부 LLM 허용·HTTP LLM 구성 후 기초설정으로 켠다. 비용·주파수는 `ai.proactive.llm-conversation-max-llm-calls-per-channel-hour`, 품질은 `ai.proactive.llm-conversation-confidence-min`. 모델 기본 `ai.llm.model=gpt-5-mini`(미지원 시 운영에서 교체). 운영에서 전체 프로액티브 크론 끄기: `ai.proactive.jobs-enabled=false`. API·테이블은 `FEATURE_SPEC` §AI 프로액티브 비서, 이관 `docs/sql/migrate_ai_phase_7_3.sql` 및 **`docs/sql/migrate_user_ai_assistant_master_toggle.sql`**(기존 설치). 운영 DB는 마이그레이션 적용 후 기초설정 키 `ai.proactive.*`(활동 최소 메시지·작업 on/off·LLM 인사이트 포함) 확인.
