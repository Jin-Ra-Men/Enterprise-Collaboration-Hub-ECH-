...
- **개발자**
  ...
  - **캘린더 Phase 6-5(iCal)·허브 UI:** `GET /api/calendar/export.ics`(본인 활성 일정·목록과 동일 기간 규칙), `POST /api/calendar/import`(multipart `employeeNo`+`file`, 건별 실패는 `skippedCount`). 코덱 `CalendarIcsCodec`(RFC 5545 최소 서브셋; 반복·종일 등 미지원은 건너뜀). 워크 허브 캘린더 패널에서 `.ics` 내보내기/가져오기 및 **제안 대기** 목록(확정·해제).
  - **AI 게이트웨이(Phase 7-1 + 7-4-0 + 7-4-1-a):** `docs/AI_GATEWAY_POLICY.md`. **기초설정 키** `ai.gateway.*`(레이트·`llm-max-input-chars`), `ai.llm.*`가 `application.yml`의 `app.ai.*`보다 우선(값이 비어 있으면 yml·환경 폴백). 관리자 **설정** 탭에서 수정 시 **재기동 없이** 반영(`AiGatewayEffectiveSettings`). 엔드포인트 `GET /api/ai/gateway/status`(※ `llmMaxInputChars`), `POST /api/ai/gateway/chat`(마스킹 후 코드포인트 상한 초과 시 잘림·감사 `inputTruncated=true`). 레이트 초과 **429**. 외부 LLM 미허용 시 **403**. HTTP LLM 미구성 시 **501**, 업스트림 오류 **502**. API 키는 관리자 화면에 평문으로 보일 수 있으므로 RBAC·노출 경로에 유의.
  - **AI 대화 보조 UI (Phase 7-2-1-a ~ 7-2-2):** 채팅 컴포저(답장 초안·최근 요약), 통합 검색 모달 **AI에게 물어보기**, 스레드 모달(댓글 초안·요약), 워크 허브 업무·칸반 설명 필드 및 상세 모달 AI. 구현 `frontend/app.js`, 안내 `FEATURE_SPEC` §AI 대화 보조.
  - **AI 프로액티브·제안함 (Phase 7-3 MVP + 후속):** 채널 옵트인·DM 차단·사용자 톤/다이제스트·**개인 `ai_assistant_enabled` 마스터 스위치**(테마 설정·`/api/me` 반영)·제안함 큐·거절 쿨다운·채널 시간당 적재 상한·적재 시 옵트인·수신자 마스터 검증. 스케줄러 `ProactiveAiSuggestionScheduler`: 활동 힌트(매시 `:07`), 다이제스트 배치(매일 09:15 `Asia/Seoul`). **LLM 대화 인사이트** `ProactiveConversationInsightScheduler`(매시 `:23`): 기본 비활성 시드 `ai.proactive.llm-conversation-insight-enabled=false`; 외부 LLM 허용·HTTP LLM 구성 후 기초설정으로 켠다. 비용·주파수는 `ai.proactive.llm-conversation-max-llm-calls-per-channel-hour`, 품질은 `ai.proactive.llm-conversation-confidence-min`. 모델 기본 `ai.llm.model=gpt-5-mini`(미지원 시 운영에서 교체). 운영에서 전체 프로액티브 크론 끄기: `ai.proactive.jobs-enabled=false`. API·테이블은 `FEATURE_SPEC` §AI 프로액티브 비서, 이관 `docs/sql/migrate_ai_phase_7_3.sql` 및 **`docs/sql/migrate_user_ai_assistant_master_toggle.sql`**(기존 설치). **로컬에서 마이그레이션을 한 번에 백필할 때** `docs/sql/migrate_bundle_local_ai_calendar.sql`(캘린더 6-4 + AI 7-3 + 마스터 토글 순·주석에 전제 명시). 운영 DB는 마이그레이션 적용 후 기초설정 키 `ai.proactive.*`(활동 최소 메시지·작업 on/off·LLM 인사이트 포함) 확인.
...
- **전역 네트워크 복구(절전 외 포함)**: `apiFetch` GET/HEAD 공통 재시도(기본 3회) + 연속 실패 임계치 감지(`apiConsecutiveFailureCount`) 시 `triggerGlobalNetworkRecovery` 실행. 복구 루틴은 소켓 재초기화(`initSocket`)·채널 목록·활성 타임라인·열린 조직도(`loadOrgChart`)·열린 워크플로(`loadWorkHubChannelMembersForAssignee`/`loadChannelWorkItems`/`loadChannelKanbanBoard`)를 재동기화하고, `#modalCalendarHub`가 열려 있으면 `loadCalendarHubPanel`도 호출한다. `window.online` 이벤트에서도 동일 경로를 사용.
- **런타임 인증/요청 타임아웃 복구**: `apiFetch`는 기본 12초 타임아웃을 사용하며, 401 응답을 받으면 `triggerAuthRecovery`가 세션 정리 후 AD 자동 로그인(`tryAdAutoLogin`)을 재시도한다. 실패 시 즉시 로그인 화면(`showLogin`)으로 전환되어 재기동 없이 복구 가능.
- **DM 멤버 패널**: DM에서는 채널 생성자에 대한 `관리자` 배지와 멤버 `내보내기` 버튼을 표시하지 않는다(`loadChannelMembers`). PUBLIC/PRIVATE 채널 멤버 패널은 기존과 동일.
- **1:1 DM 중복 생성 방지(2026-04-28)**: `startDmWithUser`/`btnConfirmCreateDm`는 생성 요청 전에 `lastSidebarChannelsSnapshot`에서 동일 상대와의 2인 DM을 탐색한다. 이미 있으면 API 생성을 건너뛰고 기존 채널로 이동해 "기존 1:1 DM이 있는데 새 DM이 생기는" UX를 차단한다.
...
- **데스크톱 GitHub 릴리즈 에셋 업로드**: 로컬에서 `cd desktop && npm run build:win` 후 `desktop/dist`에 `latest.yml`·`CSTalk-Setup-{version}.exe`·`.blockmap` 생성. `GITHUB_TOKEN`(repo releases 권한) 설정 뒤 `powershell -File ./tools/publish-electron-github-release.ps1 v1.2.11` — 태그·릴리즈 생성 및 에셋 업로드(`README.md`·`docs/DEVELOPER_README.md` 자동 업데이트 절차 참고).
...
- **채팅 입력·말풍선**: `#messageInput`·`#threadMessageInput`은 **`textarea`** — Enter 전송·Shift+Enter 개행, 타임라인은 `\n`→`<br>`; 본인 텍스트는 시각이 말풍선 **옆**(`row-reverse`), **첨부 메시지**는 시각·일괄저장이 썸네일/카드 **아래 한 줄**(시각 왼쪽·일괄저장 오른쪽). **파일 카드** 열은 이미지 번들과 같이 **최대 360px**·본인은 우측 정렬로 시각 푸터를 이미지와 맞춘다. 답장 카드(`oo에게 답장`)는 `msg-has-reply`인 내 메시지에서 본문 컨테이너 폭 기준을 고정해 사용자명·메시지 길이와 무관하게 동일 폭으로 렌더링한다. **전송 전 첨부 미리보기** `#filePreview` / `#threadFilePreview`는 각각 입력창 **위**에 두고, 목록은 항목별 제거(✕)·추가 선택 시 **기존 대기에 누적**. **텍스트 줄**은 CSS 말풍선(`--msg-bubble-*`)으로 가독성 확보; 채널 전환 시 `composerDraftByChannelId`로 미전송 입력·답글 대상·대기 첨부 복원, 로그아웃 시 `clearSession`에서 맵 비움.
...
- **네트워크 단절 후 채팅 복구**: 채널 화면에 `메시지 로드 실패`가 표시된 뒤 연결이 돌아오면, 프론트 `recoverActiveChannelTimelineIfNeeded`가 `online`·탭 가시 복귀(`visibilitychange`, 디바운스)·소켓 `connect/reconnect` 훅에서 현재 채널 `loadMessages`를 자동 재시도한다(앱 재시작 불필요). `loadMessages` 자체도 5xx/408/429·`fetch` 예외 시 최대 5회 재시도한다.
...
- **글로벌 바·업무 허브**: **대시보드** 탭은 없음. **CSTalk**(`#btnAppShellHome`)은 시작 화면 복귀. **내 일정**(`#btnTopNavCalendar`·`#btnSidebarCalendar`)은 `openCalendarHubPage`로 `#modalCalendarHub` 전환(월간 그리드·`calendarHubViewMonth`/`calendarHubViewList`·`#calendarHubBtnToolbarAdd`). 월간: **날짜 숫자**(`.calendar-hub-cell-date-hit`) → `#modalCalendarHubQuickAdd`, **일정 줄**(`.calendar-hub-cell-event-line`) → **`#modalCalendarEventDetail`**(본문 수정·삭제·참석자: `/api/users/search`·`/api/calendar/events/{id}`·`/attendees`·`/share-attendees`). 목록: 행 클릭(삭제 제외) → 동일 상세 모달. 참석자 저장 시 신규 INTERNAL(조직도 사용자) 대상 공유요청이 자동 생성되고, 수신자에게는 소켓 `calendar:share-request` 알림(토스트/OS)이 전송되며, 상세 하단 `btnCalendarEventDetailShare`로 기존 일정 재전송이 가능하다. 좌측 사이드바 `#calendarShareInboxList`는 pending 공유요청 목록을 표시하고 항목 클릭 시 캘린더 허브의 받은 공유 섹션으로 이동한다. `closeCalendarHubToMain`에서 빠른 추가·상세 모달 함께 닫음. **트러블슈팅**: 일정 클릭 시 `HttpRequestMethodNotSupportedException: Request method 'GET' is not supported` 가 나오면 **실행 중인 백엔드가 `GET /api/calendar/events/{eventId}` 를 포함하지 않는 구버전**(재빌드·재기동 누락)인 경우가 많다. DB `calendar_event_attendees` 미적용은 보통 다른 형태의 영속성 예외로 나타나며, GET 미지원 오류와는 구분된다. **프로젝트**·좌측 **업무 항목**/**칸반**은 `openWorkHubFromTopNav`·`pendingWorkHubPanelFocus`로 워크플로 모달 패널 포커스(캘린더 미포함). 상단 `워크플로우`는 채널 미선택 상태에서 자동 채널 진입하지 않고 `workflowChannelPicker`를 먼저 표시한다(채널 선택 시에만 진입). 채팅 헤더 `btnOpenWorkHub`는 현재 채널 필수. `docs/FEATURE_SPEC.md` 글로벌 바·시작 화면 절.
...
- **크림 라이트 사용자 프로필 모달**: `.profile-dl--cards` 필드는 다크 전용 선택자(`html:not([data-theme="light"])`)에서 `cream`을 제외하고, 라이트·크림 공통 또는 크림 전용으로 밝은 카드 배경을 유지한다(`styles.css`).
...
- 프론트 미확인 멘션 목록: 현재 채널이 아닌 곳에서 받은 `mention:notify`를 사용자별 localStorage 키(`ech_mention_inbox_{employeeNo}`)에 최대 100건 보관한다. 사이드바 `멘션` 섹션에서 미확인 목록만 표시하며, 항목 클릭 시 해당 채널로 이동하고 `targetMessageId`로 메시지 위치를 포커스한 뒤 목록에서 제거한다. **「업무·칸반」** 단축(토스트·목록): `openUnreadMentionWorkKanban` → `navigateToMessageThenWorkKanban` → `runMessageToWorkKanbanFlow` — 메시지 기준 업무 `POST/GET /api/messages/{id}/work-items` 후 칸반 카드가 없으면 `POST .../kanban/boards/.../columns/.../cards`로 첫 컬럼·본인 담당 즉시 생성·워크플로 칸반 포커스. 채팅 메시지 우클릭 **업무·칸반으로…** 동일 핵심(`runMessageToWorkKanbanFlow`). 허브 진입 전 `clearWorkHubPendingMaps()`로 미저장 허브 상태 초기화.
...
- 동료 **프로필 모달**(`modalUserProfile`): `GET /api/users/profile?employeeNo=`로 로드. UI는 `ECH화면설계 (9)` 계열(`.ech-profile-modal`·원형 아바타·부제 `#profileModalSubtitle`에 부서·직급 ` · ` 조합·`.profile-dl--cards`). 역할·계정 상태는 표시하지 않음. **직급**(`jobLevel`)은 항상 행(없으면 `-`), **직위**(`jobPosition`)·**직책**(`jobTitle`)은 값이 있을 때만 해당 행 표시. **DM 보내기**(`btnProfileDm`)는 `startDmWithUser`로 `POST /api/channels`에 `channelType: DM`, `dmPeerEmployeeNos: [상대 사번]`, 호환용 `createdByEmployeeNo` 등을 요청 본문에 포함해 호출하며 **실제 생성자는 JWT 사원번호**로 결정된다. 서버가 내부 이름·멤버십을 처리한 뒤 `selectChannel`로 전환(자기 자신이면 버튼 비활성). DB `channels.channel_type`은 `DM` 문자열로 저장됩니다.
- **프레즌스**: `presence:set`/`presence:update`/스냅샷 키는 **사번 문자열**. 채팅·멤버 패널·**조직도 모달** 멤버 카드(`.orgchart-member-avatar-wrap .presence-dot`)에서 `[data-presence-user]`에 사번을 두고 `refreshPresenceDots`가 갱신합니다. 조직도는 `openOrgChartModal`에서 `fetchPresenceSnapshot()` 후 멤버 그리드 렌더 시 `refreshPresenceDots()`를 호출합니다. 로컬 UX: 좌측 하단 본인 상태 버튼(`#sidebarUserStatus`)으로 **온라인·자리비움** 전환(`AWAY`는 노란 점). **자동 자리비움**: 온라인일 때 5분 무활동이면 `emitMyPresenceStatus("AWAY")`, 자리비움 중 포인터·키·휠·스크롤·탭 복귀 시 `ONLINE` 복귀(`schedulePresenceIdleAway`/`onPresenceUserActivity`/`syncPresenceIdleTimerWithMyStatus`).
- **조직도 모달 선택**: 헤더 **새로고침** 버튼은 두지 않는다. `renderOrgChartTree` 직후 `findDefaultOrgChartNodeButton`이 로그인 사용자 기준 최하위 소속(팀 `users`에 본인 사번 → `currentUser.department`와 팀명 일치 → 본부 `directMembers` → 회사 `directMembers`) 트리 버튼을 찾고, `applyOrgChartNodeSelection`으로 우측 구성원·`.is-selected`·`scrollIntoView`를 적용한다. 구성원 추가 피커(`loadOrgPicker`) 기본 팀과 동일한 우선순위다.
...
## 답글 미리보기 18자 말줄임 메모 (2026-04-29)
- 배경: 답글 UI에서 길이 제어 대상은 제목(`oo에게 답장`)이 아니라 하단 미리보기 메시지 라인
- 수정 파일:
  - `frontend/app.js`
- 조치:
  - `clipReplyPreviewText(text, maxLength=18)` 유틸을 추가해 미리보기 텍스트 길이 규칙을 통일
  - 타임라인 답글 카드 미리보기: `clipReplyPreviewText(preview, 18)` 적용
  - 입력창 답글 배너 미리보기: `clipReplyPreviewText(snippet, 18)` 적용
  - 답글 제목은 `${senderLabel}에게 답장` 원문 표시로 유지(축약 제거)
- 운영 영향:
  - 긴 미리보기 텍스트가 18자 초과 시 `…`으로 잘려 UI 폭을 안정적으로 유지
  - 향후 길이 정책 변경 시 `clipReplyPreviewText`의 길이 인자만 조정하면 됨
