...
- **개발자**
  ...
  - **AI 게이트웨이(Phase 7-1 + 7-4-0):** `docs/AI_GATEWAY_POLICY.md`. 엔드포인트 `GET /api/ai/gateway/status`(레이트 상한·`llmHttpConfigured` 포함), `POST /api/ai/gateway/chat`. **사번별 분·시간 레이트 리밋** 초과 시 **429** (`AI_GATEWAY_RATE_LIMITED`). 기본 `app.ai.allow-external-llm=false` 일 때 chat은 **403** (`AI_GATEWAY_BLOCKED`), 감사 `AI_GATEWAY_POLICY_BLOCKED`. **`citedMessageIds`가 있으면 `channelId` 필수**, 각 ID는 해당 채널 메시지이며 호출자는 멤버여야 한다. `AiGatewayPiiMasker`로 마스킹 후, `app.ai.llm.http-enabled` 및 `base-url`·`api-key` 구성 시에만 OpenAI 호환 HTTP 호출·성공 시 **200** 및 감사 `AI_GATEWAY_LLM_SUCCEEDED`; 미구성 시 **501** (`AI_GATEWAY_PROVIDER_NOT_CONFIGURED`), 업스트림 오류 시 **502**. 운영에서 허용 전 법무·보안 절차 필수.
