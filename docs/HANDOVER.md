...
- **개발자**
  ...
  - **AI 게이트웨이(Phase 7-1 + 7-4-0):** `docs/AI_GATEWAY_POLICY.md`. **기초설정 키** `ai.gateway.*`, `ai.llm.*`가 `application.yml`의 `app.ai.*`보다 우선(값이 비어 있으면 yml·환경 폴백). 관리자 **설정** 탭에서 수정 시 **재기동 없이** 반영(`AiGatewayEffectiveSettings`). 엔드포인트 `GET /api/ai/gateway/status`, `POST /api/ai/gateway/chat`. 레이트 초과 **429**. 외부 LLM 미허용 시 **403**. HTTP LLM 미구성 시 **501**, 업스트림 오류 **502**. API 키는 관리자 화면에 평문으로 보일 수 있으므로 RBAC·노출 경로에 유의.
