...
- **채팅 입력·말풍선**: `#messageInput`·`#threadMessageInput`은 **`textarea`** — Enter 전송·Shift+Enter 개행, 타임라인은 `\n`→`<br>`; 본인 메시지는 `msg-mine` 오른쪽·아바타 생략·**시각·일괄저장은 텍스트·첨부 공통으로 본문 옆 하단**(`.msg-attachment-time-inline` + 텍스트 `row-reverse`). **텍스트 줄**은 CSS 말풍선(`--msg-bubble-*`)으로 가독성 확보; 채널 전환 시 `composerDraftByChannelId`로 미전송 입력·답글 대상·대기 첨부 복원, 로그아웃 시 `clearSession`에서 맵 비움.
