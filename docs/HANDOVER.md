...
- **UI 디자인 시스템·롤백**: `docs/DESIGN_SYSTEM.md` — `frontend/styles.css`·`tailwind.config.js`, `design/` 참고물, `ech_design_version`·`design-backup/legacy-design/` 복구 절차. **화면설계 (1)~(9) ↔ 앱 구역**은 동 문서 섹션 6·마크업 훅(`.ech-region--chat`, `.ech-workhub-shell`, `.ech-region--admin`, `data-ech-design-ref`). **목업 대비 남은 갭**은 `docs/DESIGN_GAP_CHECKLIST.md`에서 구역별로 체크·우선순위 관리.
