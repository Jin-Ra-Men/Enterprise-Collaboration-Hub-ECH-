...
- **AI 게이트웨이(Phase 7-1 스텁)**: `GET /api/ai/gateway/status`, `POST /api/ai/gateway/chat`(기본 `app.ai.allow-external-llm=false` → 403; 본문에 선택 `citedMessageIds`·`channelId` 근거 검증·휴리스틱 PII 마스킹 메타). 정책 문서 `docs/AI_GATEWAY_POLICY.md`.
