# 개발자용 저장소 안내 (DEVELOPER_README)

> 루트 [README.md](../README.md)는 **제품 소개** 중심입니다. 아래는 기존 루트 README의 기술·운영 상세 전체입니다.
...
> **배포 버전(데스크톱·백엔드 JAR):** **1.2.11** (Git 태그 `v1.2.11`). 변경 요약은 `../.cursor/rules/CHANGELOG.md` 참고.
...
- **@멘션:** 입력창 `@` 자동완성은 **현재 채널·DM 멤버만**(채널 상세 `members` 기준), 저장 토큰 `@{사번|이름}`; 피멘션자에게 `mention:notify`·우하단 토스트, 클릭 시 해당 채널로 이동. 현재 보고 있지 않은 채널 멘션은 사이드바 **멘션 목록(미확인만)** 으로 관리되며 클릭 시 해당 채널/메시지로 이동. 인앱 멘션 토스트·미확인 목록에는 **업무·칸반** 단축(`runMessageToWorkKanbanFlow`): 메시지당 업무 `POST/GET /api/messages/{messageId}/work-items` 후 해당 업무에 칸반 카드가 없으면 `POST .../kanban/boards/{boardId}/columns/{columnId}/cards`로 첫 컬럼에 즉시 생성; 채팅 메시지 우클릭 **업무·칸반으로…** 동일
...
- **파일 공유:** 채널별 업로드(multipart `file` + 선택 `preview`)·목록·다운로드(`variant=original|preview`, JWT), 디스크 경로에 워크스페이스·채널 식별 세그먼트 포함
- **이미지 첨부:** 채팅 타임라인에서는 **2열 그리드** 썸네일(한 줄 2개·개행), 썸네일 클릭 시 **라이트박스**에서 서버 **미리보기**(`GET .../preview`) 우선·원본 보기·다운로드; 업로드 시 클라이언트 압축본을 `preview`로 보내면 DB·메시지에 `previewSizeBytes` 반영. 다운로드는 **원본** vs **미리보기(압축)** 선택 시 각각 저장 용량 표시, 구버전 첨부(미리보기 없음)는 **약 512KB 이상**일 때만 원본 vs 브라우저 JPEG 재인코딩(`modalImageDownloadChoice`, GIF/SVG는 원본만). 클립보드 **붙여넣기(Ctrl+V)** 로 이미지 **여러 장** 동시 첨부 가능
- **첨부 UX:** 채팅 영역 **드래그 앤 드롭**·파일 선택 **다중(`multiple`)**·붙여넣기로 **여러 파일**을 한 번에 올린 뒤 순차 전송; 비이미지는 **카드**(저장·저장 후 열기), 이미지는 **그리드**로 표시 + 햄버거 메뉴 **첨부파일**·**이미지 모아보기**(채널/DM 공통; 전체 탭은 비이미지 첨부만, 이미지는 이미지 탭 전용). **같은 분에 연속으로 올린 FILE 메시지**(이미지·문서 혼합 가능)는 **한 묶음**으로 보이며 **일괄저장**은 **ZIP 한 파일**로 내려받음(JSZip). **FILE 직후** 다음 메시지는 **새 발화**(아바타 행)로 이어짐
- **칸반 보드:** 워크스페이스별 보드·컬럼·카드 CRUD, 담당자 지정, 상태/컬럼 이동 이력 API
- **채팅→업무:** 메시지 ID 기준 업무 항목 생성·조회(메시지와 1:1 링크). 업무에 **마감(`dueAt`)·우선순위(`priority`: LOW/NORMAL/HIGH)** 필드 지원; 수정 시 마감 제거는 `clearDueAt: true`
- **채널 워크플로우:** 채널 헤더 `📋`에서 **워크플로우**(업무·칸반)를 한 모달에서 조회하고, 신규/상태/컬럼 이동/순서 변경은 **저장**으로 반영, 항목 **삭제(✕)**, 비활성 업무 **복원·완전 삭제**는 목록·상세에서 예약 후 **저장** 시에만 서버 반영(예약 행은 배지로 표시), 칸반 **채널 멤버**(본인 포함) 담당 지정·자동완성(이름/조직·직급 구분, 키보드 선택 지원), 칸반 카드는 **제목·본문 등 카드 본체**를 잡아 컬럼 안·**컬럼 간** 드래그(검색창·컬럼 `<select>`·버튼 위에서는 드래그가 시작되지 않음)하며, 드롭 후 컬럼 `<select>`는 DOM 재생성으로 Chrome 표시 불일치를 완화한다. 저장 시 컬럼에 맞게 카드 `status`도 함께 반영. **저장** 성공 시 사이드바 **내 업무 항목** 목록도 즉시 갱신
- **내 할 일(사이드바):** `GET /api/work-items/me/todos`로 지연·오늘 마감(Asia/Seoul)·멘션 연계 메시지·담당 칸반 버킷과 **`badgeCounts`(지연·48h 임박 총건)**를 채운다. 멘션 연계 행은 채팅으로 전환해 원본 메시지를 포커스하고, 그 외 버킷은 워크플로 모달만 연다
- **업무/칸반 상세 편집:** 목록 카드 클릭 시 상세 모달에서 제목·설명·상태·**우선순위·마감**을 수정하고, 칸반 카드는 담당자 추가/해제까지 지원하며 변경은 저장 버튼에서 일괄 반영
...
- **채팅 UX:** 날짜 변경 시 구분선(pill), 비이미지 첨부는 **카드**, 이미지는 **2열 그리드**+클릭 시 라이트박스, DM **사이드바**·**채팅 헤더**에 상대 **프레즌스 점**, 좌측 하단 본인 상태는 실시간 프레즌스와 동기; 입력은 **Enter 전송·Shift+Enter 줄바꿈**, 본문 줄바꿈 표시, **본인 메시지**는 오른쪽 정렬·아바타 숨김, **채널별** 작성 중(미전송 텍스트·답글·대기 첨부) 저장; 미읽음 **「새 메시지」** 라인은 입력·전송 시 사라짐
...
- **AD 자동 로그인 (Electron 전용):** AD 조인된 사내 PC에서 앱 실행 시 Windows 계정명으로 자동 인증 (`POST /api/auth/ad-login`); 미등록·INACTIVE 계정은 일반 로그인 화면으로 폴백
- **사용자 관리:** 관리자 메뉴 "사용자 관리"에서 직원 계정 등록·수정·삭제, ACTIVE/INACTIVE 상태 전환, 부서/직급/직위/직책 조직 정보 일괄 저장
- **통계 시각화:** 접속자 수, 메시지 전송량 등 주요 지표를 그래프로 시각화 (Chart.js 활용)
- **권한 관리:** 부서별 권한 설정, 퇴사자 계정 비활성화 및 보안 로그 감사
...
- 조직도 DDL·마이그레이션: `./sql/create_org_groups.sql`, `./sql/create_org_group_members.sql`,
...
- 캘린더 제안·AI 프로액티브 로컬 백필(묶음): `./sql/migrate_bundle_local_ai_calendar.sql` — 또는 개별 `./sql/migrate_calendar_phase_6_4.sql`, `./sql/migrate_ai_phase_7_3.sql`, `./sql/migrate_user_ai_assistant_master_toggle.sql`
...
- **AI 게이트웨이(Phase 7-1 + 7-4-0 + 7-4-1-a)**: `GET /api/ai/gateway/status`(※ `llmMaxInputChars`), `POST /api/ai/gateway/chat`(레이트·정책·마스킹 후 입력 코드포인트 상한·LLM은 **`app_settings` 우선**, 빈 값은 `app.ai.*`/환경 폴백). 관리자 `GET|PUT /api/admin/settings`. `docs/AI_GATEWAY_POLICY.md`.
- **AI 대화 보조 UI (7-2-1-a ~ 7-2-2)**: `frontend/app.js` — 컴포저 ✨답장 초안·요약, 검색 모달 **AI에게 물어보기**, 스레드 모달 AI, 워크 허브·칸반 설명 textarea 채움. 근거 `citedMessageIds`·단일 채널 검색 결과 제약은 백엔드 검증과 동일.
- **AI 프로액티브·제안함 (7-3 MVP + 후속)**: `GET|PUT /api/channels/{channelId}/ai-assistant/preference`, `GET|PUT /api/me/ai-assistant/preferences`, `GET /api/me/ai-suggestions`, `POST /api/me/ai-suggestions/{id}/dismiss|acknowledge`. 스케줄 `ProactiveAiSuggestionScheduler`(활동 힌트·다이제스트), 선택 **`ProactiveConversationInsightScheduler`**(대화 인사이트 LLM·기본 비활성 `ai.proactive.llm-conversation-insight-enabled`). 도메인 `com.ech.backend.domain.aiassistant`. 이관 SQL `docs/sql/migrate_ai_phase_7_3.sql`.
- **캘린더(개인·공유·제안·충돌·iCal·참석자)**: `GET /api/calendar/events?employeeNo=&from=&to=`, `GET /api/calendar/events/{eventId}?employeeNo=`(단건·`attendees` 포함), `POST /api/calendar/events`(직접 생성은 `createdByActor` USER만), `GET /api/calendar/events/conflicts?employeeNo=&startsAt=&endsAt=&excludeEventId=`, `GET /api/calendar/suggestions?employeeNo=&status=`, `POST /api/calendar/suggestions`, `POST /api/calendar/suggestions/{id}/confirm|dismiss?employeeNo=`, `GET /api/calendar/export.ics?employeeNo=&from=&to=`, `POST /api/calendar/import`(multipart `employeeNo`, `file`), `PUT /api/calendar/events/{eventId}?employeeNo=`, `PUT /api/calendar/events/{eventId}/attendees?employeeNo=`(참석자 전체 치환·INTERNAL/EXTERNAL·신규 INTERNAL 자동 공유요청), `POST /api/calendar/events/{eventId}/share-attendees?employeeNo=`(INTERNAL 참석자 일괄 전송, 요청/생성/스킵 건수 응답), `DELETE /api/calendar/events/{eventId}?employeeNo=`, `POST /api/channels/{channelId}/calendar/shares`, `GET /api/calendar/shares/incoming?employeeNo=`, `GET /api/calendar/shares/outgoing?employeeNo=`, `POST /api/calendar/shares/{shareId}/accept|decline?employeeNo=` (JWT 본인 검증·채널 멤버십·타인 반영은 공유 경로만). 참석자 스키마: `docs/sql/migrate_calendar_event_attendees.sql`
- **채널 자료실**: `GET /api/channels/{channelId}/library/folders`, `POST|PATCH|DELETE .../library/folders/...`, `PATCH .../library/files/{fileId}`; 파일 목록 `GET .../files` 에 `libraryFolderId`, `libraryUncategorizedOnly` 쿼리 (멤버·`employeeNo`)
...
- 첨부 저장 경로(특히 UNC)가 백엔드 서비스 계정으로 쓰기 가능한지: `GET /api/admin/storage/probe` (ADMIN JWT). 운영 절차: `./DEPLOYMENT_WINDOWS.md`
...
- **레거시 ECH 제거**: 설치 시 `desktop/nsis/installer.nsh`가 기본 경로의 **ECH**(`Uninstall ECH.exe /S`)를 먼저 무인 제거한 뒤 CSTalk를 설치한다. 자동 업데이트로 새 설치 파일이 실행될 때도 동일하다.
- **빌드 실패(`app.asar` 사용 중 / EPERM)**: CSTalk·`electron.exe`는 종료했는데도 같은 오류가 나면 **Cursor·VS Code를 완전히 종료**한 뒤, **저장소 밖에서 연 PowerShell**로만 `npm run build:win` 실행해 보세요(에디터가 `desktop/dist` 안 파일을 잠그는 경우가 많습니다). **Windows Defender** 실시간 검사가 `app.asar`를 붙잡는 경우도 있어, 프로젝트 경로를 일시 제외하거나 재시도합니다(`clean-dist.cjs`는 자동으로 몇 번 재시도함). 그래도 안 되면 **작업 관리자 → 성능 → 리소스 모니터 → 연결된 핸들**에서 `app.asar`로 검색해 잡는 프로세스를 확인합니다. 우회: 출력만 다른 폴더로 빌드 후 설치 파일만 옮김 — `cd desktop` 후 `npx electron-builder --win nsis --publish never --config.directories.output=dist-build-temp` → 생성된 `CSTalk-Setup-*.exe`·`latest.yml` 등을 원하는 위치로 복사. **수동으로 `dist`를 이미 비웠다면** `CSTALK_SKIP_DIST_CLEAN=1 npm run build:win` 으로 삭제 단계를 건너뛸 수 있습니다. **`desktop/dist` 자체를 지울 수 없을 때**(잠금으로 `clean-dist`가 계속 실패)에는 `CSTALK_SKIP_DIST_CLEAN=1`과 함께 `npx electron-builder --win nsis --publish never --config.directories.output=dist-release-<버전>`처럼 **별도 출력 폴더**로 빌드하면 `desktop/dist-release-*/CSTalk-Setup-*.exe`가 생성됩니다(로컬 산출물은 `.gitignore`에 `desktop/dist-release-*/`로 제외).
