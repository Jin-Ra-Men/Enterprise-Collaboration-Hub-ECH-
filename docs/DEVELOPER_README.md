...
- **캘린더(개인·공유·제안·충돌)**: `GET /api/calendar/events?employeeNo=&from=&to=`, `POST /api/calendar/events`(직접 생성은 `createdByActor` USER만), `GET /api/calendar/events/conflicts?employeeNo=&startsAt=&endsAt=&excludeEventId=`, `GET /api/calendar/suggestions?employeeNo=&status=`, `POST /api/calendar/suggestions`, `POST /api/calendar/suggestions/{id}/confirm|dismiss?employeeNo=`, `PUT /api/calendar/events/{eventId}?employeeNo=`, `DELETE /api/calendar/events/{eventId}?employeeNo=`, `POST /api/channels/{channelId}/calendar/shares`, `GET /api/calendar/shares/incoming?employeeNo=`, `GET /api/calendar/shares/outgoing?employeeNo=`, `POST /api/calendar/shares/{shareId}/accept|decline?employeeNo=` (JWT 본인 검증·채널 멤버십·타인 반영은 공유 경로만)
