...
- **AI 게이트웨이(Phase 7-1 + 7-4-0)**: `GET /api/ai/gateway/status`, `POST /api/ai/gateway/chat`(레이트·정책·LLM은 **`app_settings` 우선**, 빈 값은 `app.ai.*`/환경 폴백). 관리자 `GET|PUT /api/admin/settings`. `docs/AI_GATEWAY_POLICY.md`.
