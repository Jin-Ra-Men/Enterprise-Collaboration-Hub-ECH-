...
- **AI 게이트웨이(Phase 7-1 + 7-4-0)**: `GET /api/ai/gateway/status`, `POST /api/ai/gateway/chat`(레이트 리밋·기본 `allow-external-llm=false` → 403; 선택 `app.ai.llm.*`로 OpenAI 호환 HTTP·성공 시 200; 한도 초과 429). `citedMessageIds`·`channelId` 근거 검증·PII 마스킹. `docs/AI_GATEWAY_POLICY.md`.
