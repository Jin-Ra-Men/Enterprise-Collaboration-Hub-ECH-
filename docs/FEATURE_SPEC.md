...
- 관련 화면/경로: `frontend/index.html` — 전역 **내 일정** `#modalCalendarHub`(글로벌 바「내 일정」·사이드바 허브 단축 동일 진입). `frontend/app.js`: `openCalendarHubPage`, `loadCalendarHubPanel`, `ensureCalendarHubBindings`, `CALENDAR_HUB_IDS`. 워크플로(`#modalWorkHub`)에는 캘린더 패널 없음. **iCal** `#calendarHubBtnIcsExport`·`calendarHubBtnIcsImport`·`calendarHubIcsFileInput`; **제안 대기** `#calendarHubSuggestionsList`(확정·해제); 공유 출처 `#calendarHubShareChannelSelect`.
- 관련 API:
