...
## 협업 툴 인식 강화 (제품 방향 — 플랜 이행)

- 목적: 메신저가 아닌 **협업 허브** 인식을 위한 **우선 사용자 여정**, **지식 축(자료실 1차)**, **캘린더+AI 공동 설계 원칙**, **AI 거버넌스·프로액티브·비용**을 개발 기준으로 고정한다.
- 사용자: PM·개발·운영(로드맵·스프린트 계획 시 참조)
- 관련 문서: [COLLABORATION_TOOL_DIRECTION.md](./COLLABORATION_TOOL_DIRECTION.md)(갭 목록·정책 표), [ROADMAP.md](./ROADMAP.md) `Phase 2.7`·`6-4`·`7-*`
- 구현 상태: 본 섹션은 **방향 문서**이며, 구체 API·UI는 해당 Phase 완료 시 기능 템플릿 절로 이전·보강한다.
...
- 관련 화면/경로: `frontend/index.html` — 전역 **내 일정** `#modalCalendarHub`. **월간** 탭: 격자 칸에 `(Asia/Seoul 시각) 제목` 형태로 일정 표시. 칸은 **`div.calendar-hub-cell`**: **날짜 숫자** 버튼(`.calendar-hub-cell-date-hit`) 클릭 → 해당 일 기준 `#modalCalendarHubQuickAdd`, **일정 줄** 버튼(`.calendar-hub-cell-event-line`) 클릭 → **`#modalCalendarEventDetail`**(제목·설명·시작/종료·저장·삭제·참석자). **목록보기** 탭: 표 **`#calendarHubEventTableBody`**에서 행 클릭(삭제 버튼 제외) 시 동일 상세 모달. 좌측 사이드바에 **받은 일정 공유**(`#calendarShareInboxList`) 섹션을 두어 pending 공유요청을 즉시 확인하고 캘린더 허브로 이동할 수 있다. 참석자: 전사 `GET /api/users/search?q=` 자동완성으로 **INTERNAL** 추가, 이름(+선택 이메일)로 **EXTERNAL** 추가 후 `PUT /api/calendar/events/{id}/attendees`로 치환 저장. 참석자 저장 시 신규 INTERNAL(조직도 사용자)은 해당 일정의 공유요청이 자동 생성되고, 상세 모달 버튼으로 기존 일정을 재전송할 수 있다. 상단 **일정 추가**·달 이동·`#calendarHubViewMonth`/`#calendarHubViewList`. 허브를 닫을 때 `#modalCalendarEventDetail`도 정리(`closeCalendarHubToMain`). `frontend/app.js`: `openCalendarEventDetailModal`, `renderCalendarHubMonthGrid`, `renderSidebarCalendarShareInbox`, `calendarHubFormatSeoulMdHm`, `renderCalendarHubEventTable`, `calendarHubViewMode`. 워크플로(`#modalWorkHub`)에는 캘린더 없음. **iCal**·제안·공유 UI 동일.
- 관련 API:
  ...
  - `PUT /api/calendar/events/{eventId}/attendees?employeeNo=` — 참석자 **전체 치환**(본문 `{ attendees: [{ attendeeType: INTERNAL|EXTERNAL, employeeNo?, displayName, email? }] }`, INTERNAL은 등록 사용자 사번 필수·중복 불가, EXTERNAL은 표시명 필수·사번 불가, 최대 50명, 신규 INTERNAL 자동 공유요청)
  - `POST /api/calendar/events/{eventId}/share-attendees?employeeNo=` — INTERNAL 참석자 일괄 전송(요청/생성/스킵 건수 응답)
  ...
  - `GET /api/calendar/export.ics?employeeNo=&from=&to=` — 본인 **활성** 일정만 ICS 본문(`text/calendar`; 목록 API와 동일 기본 구간 규칙·첨부 권장 파일명 `cstalk-calendar.ics`)
  - `POST /api/calendar/import` (`multipart/form-data`: `employeeNo`, `file`) — ICS 파싱 후 건별 **직접 생성과 동일** 경로로 저장; 응답 `importedCount`·`skippedCount`(파싱·검증 실패 분). **지원 서브셋**: 단일 `VEVENT`, `DTSTART`/`DTEND` 또는 `DURATION`, UTC·숫자 오프셋 또는 `TZID`+로컬 컴팩트 시각; **비지원·건너뜀**: `RRULE`, 종일(`VALUE=DATE`), 파일 **512KB**·**64건** 상한
- 관련 Socket 이벤트: `calendar:share-request` (수신자 실시간 공유요청 알림 토스트/OS 알림)
...
- 테스트 기준: `CalendarApiTest` — 직접 일정 생성·목록, 채널 공유·수락 후 출처 채널명, 제안 확정·직접 생성 AI 거부·충돌 검사·**iCal 내보내기/가져오기**
- 비고: **제안→확정**은 타인 달력을 바꾸지 않으며, 타인 반영은 여전히 **공유→수락**만 허용한다. DB 이관: `docs/sql/migrate_calendar_phase_6_4.sql`. 외부 동기·OAuth 등은 Phase 8.
...
- **대용량 파일 다운로드 진행(프론트)**: `GET .../download` 응답의 `Content-Length`가 약 **512KB 이상**이면 `fetch` 본문을 스트림으로 읽으며(`responseToBlobWithProgress`) 화면 하단 **`#fileDownloadStatusBar`**에 `다운로드 중… N%` 등을 표시(길이 헤더가 없으면 퍼센트 없이 문구만). 일반 파일 저장·ZIP 일괄·클라이언트 JPEG 재압축 시 원본 수신 구간 등에 동일 패턴 적용.
- 관련 API:
...
- 입력/출력:
  ...
  - **사이드바 조직도 모달**(`modalOrgChart`)·**구성원 추가 피커**·**관리자 사용자 관리** 표 모두 동일 정렬(`sortOrgDirectoryMembers`): `JOB_TITLE.sort_order`(직책) → `JOB_LEVEL.sort_order`(직급) → 관리자 지정 사용자 위치 순번(`directorySortOrder`) → 이름 가나다.
...
- **상단 검색 폭 기준**: `appHeaderSearchInput` 래퍼(`.app-shell-global-search`)는 사이드바 폭(324px) 기준선 밖으로 튀어나오지 않도록 축소된 고정폭(236px)으로 유지한다.
  - **대용량 첨부·이미지(프론트)**: 미리보기는 큰 이미지를 다운스케일한 blob URL로 표시해 UI 멈춤을 줄임; 약 **2MB 초과** 이미지는 업로드 전 최대 변 길이 4096px·JPEG 재압축(애니 GIF는 원본 유지); 업로드는 `XMLHttpRequest`로 **진행률** 표시, 성공 후 타임라인·파일 허브 갱신은 병렬 처리(`buildImagePreviewObjectUrl`, `maybeCompressImageForUpload`, `uploadFileWithProgress`). **전송 취소**: 대기를 비울 때(항목별 ✕로 모두 제거해 `clearFilePreview`/`clearThreadFilePreview`가 호출될 때 등) 진행 중 업로드 **`abort()`**·`fileUploadSessionId`로 압축 단계까지 중단 가능하게 하고, 순차 전송 루프는 대기 목록 **스냅샷** 기준으로 취소 시 이후 파일 전송을 하지 않음.
  - **줄바꿈**: 메인·스레드 입력은 **`textarea`** — **Enter** 전송, **Shift+Enter** 개행; 서버에 저장된 본문의 줄바꿈은 타임라인에서 `<br>` 로 표시(`formatMessageWithMentions` 후 `\n` 치환)
  - **본인 말풍선**: 발신자가 로그인 사용자와 같으면 **오른쪽 정렬**·**아바타·이름 행 숨김**(`msg-mine`·`msg-body--mine`), 텍스트·파일·이미지 첨부 행 공통. **텍스트**의 시각은 말풍선과 **한 줄**(`inline-flex`·`row-reverse`). **이미지·파일 첨부**는 썸네일·카드 **아래** 별도 줄(`.msg-attachment-meta-below`)에 두며, **시각은 왼쪽**, 첨부 묶음이면 **일괄저장**은 같은 줄 **오른쪽**(`justify-content: space-between`). **파일 카드** 열은 **이미지 번들**과 동일하게 최대 폭 `min(100%, 360px)`·본인 메시지는 `margin-left: auto`로 맞춰 시각 줄이 이미지와 동일하게 보이게 한다. **텍스트 줄**은 배경 말풍선(보라 톤/라이트는 인디고 틴트)·테두리·그림자로 구역을 구분하고, 본인 말풍선 **안쪽 본문은 좌측 정렬**로 읽기 쉽게 맞춤
  - **전송 전 첨부 미리보기**: 메인 `#filePreview`·스레드 `#threadFilePreview`는 입력 행(`.ech-composer-glass` / `.thread-composer-controls`) **바로 위**에 전체 너비로 표시(컴포저는 세로 스택). 대기 목록은 `#filePreviewList` / `#threadFilePreviewList`에 **항목별 행**(썸네일·파일명·개별 ✕)으로 렌더링하며, **추가 선택·드롭·붙여넣기**는 기존 대기가 있으면 **누적(append)**. 대기 비우기는 **항목별 ✕**로만 수행한다.
...
- **처리**: `desktop/main.js` `powerMonitor.on("resume" | "unlock-screen")` → 렌더러 `ech-system-resume` IPC → `preload` `electronAPI.onSystemResume` → `frontend/app.js` `setupElectronSystemResumeRecovery`.
  ...
  - 런타임 인증 복구: `apiFetch`가 401을 받으면 `triggerAuthRecovery`가 세션을 정리하고 Electron AD 자동 로그인을 재시도(실패 시 로그인 화면 전환). 또한 공통 요청 타임아웃 12초를 적용해 장시간 반쯤 끊긴 연결 대기를 줄인다.
...
- 주요 기능:
  - **테이블 뷰**: 사원번호/이름/이메일/부서/직급/직위/직책/위치(사용자별 순번)/역할/상태 표시
...
- 데이터 모델:
  - `users` 테이블: `employee_no`, `email`, `name`, `role`(MEMBER/ADMIN), `status`(ACTIVE/INACTIVE), `directory_sort_order`(조직도 노출용 사용자별 순번)
...
## 답글 미리보기 말줄임 처리 (2026-04-29)
- 목적: 답글 카드/입력창 배너에서 **메시지 미리보기 줄**이 길 때 18자 기준으로 안정적으로 축약
- 관련 경로:
  - `frontend/app.js`
- 변경 사항:
  - `clipReplyPreviewText(text, maxLength)` 유틸을 추가해 미리보기 텍스트를 공통 규칙으로 생성
  - 답글 제목(`oo에게 답장`)은 축약하지 않고 원문 그대로 표시
  - 타임라인 답글 카드 미리보기와 입력창 답글 배너 미리보기에 `clipReplyPreviewText(..., 18)` 적용
  - 18자 초과 시 뒷부분을 `…`으로 처리하고, 공백은 단일 공백으로 정규화
