...
- **상단 검색 폭 기준**: `appHeaderSearchInput` 래퍼(`.app-shell-global-search`)는 사이드바 폭(324px) 기준선 밖으로 튀어나오지 않도록 축소된 고정폭(236px)으로 유지한다.
  - **대용량 첨부·이미지(프론트)**: 미리보기는 큰 이미지를 다운스케일한 blob URL로 표시해 UI 멈춤을 줄임; 약 **2MB 초과** 이미지는 업로드 전 최대 변 길이 4096px·JPEG 재압축(애니 GIF는 원본 유지); 업로드는 `XMLHttpRequest`로 **진행률** 표시, 성공 후 타임라인·파일 허브 갱신은 병렬 처리(`buildImagePreviewObjectUrl`, `maybeCompressImageForUpload`, `uploadFileWithProgress`). **전송 취소**: 전체 취소 시 진행 중 업로드 **`abort()`**·`fileUploadSessionId`로 압축 단계까지 중단 가능하게 하고, 순차 전송 루프는 대기 목록 **스냅샷** 기준으로 취소 시 이후 파일 전송을 하지 않음.
  ...
  - **전송 전 첨부 미리보기**: 메인 `#filePreview`·스레드 `#threadFilePreview`는 입력 행(`.ech-composer-glass` / `.thread-composer-controls`) **바로 위**에 전체 너비로 표시(컴포저는 세로 스택). 대기 목록은 `#filePreviewList` / `#threadFilePreviewList`에 **항목별 행**(썸네일·파일명·개별 ✕)으로 렌더링하며, **추가 선택·드롭·붙여넣기**는 기존 대기가 있으면 **누적(append)**. **전체 취소**는 기존처럼 한 번에 비움.
