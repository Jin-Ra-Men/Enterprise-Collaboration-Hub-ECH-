...
- 입력/출력:
  ...
  - UI: 레이아웃은 **업무 항목 패널 위 → 칸반 보드 패널 아래** 한 열 세로 배치(`work-hub-body` flex column). 모달 본문 전체 스크롤. 신규 카드는 **저장된 업무**를 선택한 뒤 큐에 넣고, 저장 시 생성. 업무 **✕**는 소프트 삭제(목록 회색·취소선), 상세에서 복원·완전 삭제
  ...
  - 칸반 카드는 컬럼 내 세로 순서뿐 아니라 **컬럼 간 드래그앤드롭**으로 이동 가능. **드래그는 카드 제목 줄의 `⋮⋮` 핸들(`.kanban-card-drag-handle`, `draggable="true"`)에서만 시작**한다. 카드 루트(`article`)를 `draggable`로 두면 Chrome 등에서 이동 후에도 행 하단 컬럼 `<select>` 표시가 실제 컬럼과 어긋나는 브라우저 동작이 있어 핸들 전용으로 분리했다. 드롭 시 출발·도착 컬럼의 순서·컬럼 임시 상태를 반영하고, **저장** 시 컬럼이 바뀐 카드에 대해 `PUT .../kanban/cards/{id}`에 `columnId`와 함께 컬럼에 대응하는 `status`(`OPEN`/`IN_PROGRESS`/`DONE`, 기본 3컬럼은 정렬 순 0·1·2)를 함께 보낸다
