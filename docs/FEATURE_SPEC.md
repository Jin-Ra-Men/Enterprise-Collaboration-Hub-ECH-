...
- 입력/출력:
  ...
  - 파일 업로드 성공 시: **비이미지**는 메시지 행 안에 **카드 리스트**(아이콘·파일명·크기 + **저장** / **저장 후 열기**만; **뷰어로 열기 없음**). **저장 후 열기**: **브라우저**에서는 blob **새 탭**(브라우저가 미리보기 가능한 형식만 탭에서 표시). **ECH 데스크톱(Electron)** 에서는 `%TEMP%\\ech-open\\` 에 임시 저장 후 `shell.openPath`로 **OS 기본 연결 앱** 실행(Windows `.txt` 등은 사용자가 연결한 메모장·VS Code 등). IPC `ech-open-temp-file-default-app` / `electronAPI.openTempFileWithDefaultApp` **이미지**는 같은 묶음 안에서 **2열 그리드** 썸네일(한 줄에 2개, 넘치면 다음 줄) — 썸네일 클릭 시 `openChannelImageLightbox`(크게 보기·모달에서 다운로드). **연속 FILE 메시지**(같은 분·같은 발신자·스레드 댓글 없음)는 `tryConsumeFileAttachmentGroup` → `createFileAttachmentGroupRowFromMsgs`로 **한 말풍선**에 묶고, 이미지 블록+파일 카드가 함께 있으면 위·아래로 배치, **일괄저장**은 묶음 전체 파일 순차 저장
  - **연속 이미지(같은 분·같은 발신자)**: 서버는 파일별 `FILE` 메시지로 저장하되, 타임라인에서는 **2장 이상**이면 **2열 그리드**로 묶어 표시(스레드 댓글이 달린 메시지는 묶지 않음). 하단 **일괄저장**은 각 파일 **원본**을 순차 다운로드(`batchDownloadChannelImageFiles`)
