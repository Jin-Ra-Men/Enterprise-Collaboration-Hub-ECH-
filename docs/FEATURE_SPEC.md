...
- 관련 API:
  ...
  - `GET /api/user-directory/organization-filters` — ACTIVE 사용자의 `company_key` 그룹별 대표 `company_name`으로 조직도 팝업 **회사 셀렉트 옵션**(`filterValue` + `label`) 제공. 첫 항목은 항상 `ORGROOT` / 전체.
  - `GET /api/user-directory/organization?companyKey=...`(선택) — ACTIVE 사용자를 **회사 → 본부 → 팀(부서)** 3단계 트리로 그룹화. 응답 루트는 `data.companies[]`이며, 각 회사에 `divisions[]` → 각 본부에 `teams[]` → 각 팀에 `users[]`가 포함됩니다. 조직 깊이는 `users.company_name` / `users.division_name` / `users.team_name`을 그대로 사용하며, 값이 비어 있으면 해당 레벨은 `미지정` 버킷으로 분류합니다. **회사 필터**: 쿼리 `companyKey`가 없거나 빈 값이면 전체; `ORGROOT`도 전체(그룹사 공용). 그 외 `GENERAL` / `EXTERNAL` / `COVIM365` 등은 `users.company_key`와 일치하는 행만 트리에 포함(레거시 null `company_key`는 `GENERAL` 필터에 포함). DB에 조직 컬럼이 없으면 `docs/sql/migrate_users_add_org_columns.sql`·`migrate_users_company_key.sql` 적용 후 `seed_test_users.sql`(UPSERT) 또는 백필 스크립트로 채운다. (구) `/api/users/organization` 은 정적 `/**` 매핑과 겹칠 수 있어 분리. 백엔드는 `add-mappings: false` + 명시적 프론트 3파일만 서빙해 `/api/**` 404를 방지
