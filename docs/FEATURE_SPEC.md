...
- 입력/출력:
  ...
  - UI: 레이아웃은 **업무 항목 패널 위 → 칸반 보드 패널 아래** 한 열 세로 배치(`work-hub-body` flex column). 모달 본문 전체 스크롤. 신규 카드는 **저장된 업무**를 선택한 뒤 큐에 넣고, 저장 시 생성. 업무 **✕**는 소프트 삭제(목록 회색·취소선). 비활성 업무의 **복원**·**완전 삭제**는 **업무 목록 행** 또는 **업무 상세**에서 선택 가능하며, 모두 `workHubPendingWorkRestoreIds` / `workHubPendingWorkPurgeIds`에만 쌓였다가 **업무·칸반 모달 저장(`flushWorkHubSave`)** 시에만 API 호출(즉시 DB 반영 아님). 목록에서 **복원 예정**(녹색 배지)·**완전 삭제 예정**(적색 배지)으로 표시, **취소** 버튼으로 대기 큐에서 제거. 공통 확인창 `#modalAppDialog`는 z-index를 상세 모달보다 높게 두어 **완전 삭제 확인**이 상세 모달에 가리지 않음
...
- 입력/출력:
  ...
  - 파일 업로드 성공 시: 일반 텍스트 메시지와 동일한 **메시지 행**(아바타·발신자·시간) 안에 **카드 리스트**로 표시 — 아이콘(유형)·파일명·크기 + **저장**(`maybeDownloadChannelImageWithChoice` 등) / **저장 후 열기**(blob 다운로드 + 새 탭) / **뷰어로 열기**(이미지는 `openChannelImageLightbox`, 그 외는 새 탭 blob). **연속 FILE 메시지**(같은 분·같은 발신자·스레드 댓글 없음)는 `tryConsumeFileAttachmentGroup`으로 묶어 **일괄저장** 제공
  - **연속 이미지(같은 분·같은 발신자)**: 서버는 파일별 `FILE` 메시지로 저장하되, 타임라인 렌더 시 **2장 이상**이면 **한 줄 가로 스트립**으로 묶어 표시(스레드 댓글이 달린 메시지는 묶지 않음). 하단 **일괄저장**은 각 파일 **원본**을 순차 다운로드(`batchDownloadChannelImageFiles`)
