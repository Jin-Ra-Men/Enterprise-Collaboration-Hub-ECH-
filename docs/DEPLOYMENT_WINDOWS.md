...
클라이언트 접속: `http://cstalk.co.kr:8080` (hosts 파일로 `cstalk.co.kr → 192.168.11.168` 지정)
...
cd backend
./gradlew.bat bootJar
# 결과물: backend/build/libs/cstalk-backend-{버전}.jar (버전은 backend/build.gradle 의 version)
```
...
### 3-1. 배포 폴더 생성
```powershell
New-Item -ItemType Directory -Force C:\CSTalk\backend
New-Item -ItemType Directory -Force C:\CSTalk\realtime
New-Item -ItemType Directory -Force C:\CSTalk\logs
New-Item -ItemType Directory -Force C:\CSTalk\storage
New-Item -ItemType Directory -Force C:\CSTalk\releases
New-Item -ItemType Directory -Force C:\CSTalk\releases\desktop
```

### CSTalk 데스크톱 설치 경로(Windows)

- NSIS **`perMachine: true`** 로 **전역(All Users) 설치**이며, 기본 위치는 **`%PROGRAMFILES%\CSTalk\`** (예: `C:\Program Files\CSTalk\CSTalk.exe`) 입니다. 설치 시 **관리자(UAC) 승인**이 필요합니다.
- **`cstalk-server.json`**(선택, `serverUrl` / `updateBaseUrl`)은 앱이 아래 **먼저 존재하는 경로**를 읽습니다.
...
- **한 줄 요약**: 설치된 CSTalk 데스크톱 앱이 **주기적으로(또는 실행 시)** 사내 백엔드에 올려둔 **「최신 버전 안내」(`latest.yml`)** 과 **설치 파일(NSIS `.exe`)** 을 받아, 새 버전이 있으면 사용자에게 알리고 설치할 수 있게 하는 방식입니다. 인터넷의 GitHub Releases 대신 **회사망 안의 웹 서버(백엔드와 동일 호스트)** 가 그 역할을 합니다.
...
- **흐름**:
  1. 사용자 PC의 **CSTalk.exe**가 `cstalk-server.json`의 **`serverUrl`**(또는 `updateBaseUrl`)을 읽습니다.
...
| 구성 요소 | 역할 |
|---|---|
| **CSTalk 데스크톱 (클라이언트)** | `electron-updater`로 메타·설치 파일 수신, 설치 안내 UI |
| **`cstalk-server.json`** | `serverUrl` 등으로 **업데이트를 어디서 받을지** 지정 (없으면 기본은 GitHub — 내부망에서는 실패 가능) |
| **`latest.yml`** | 버전·파일명·해시 등 메타데이터 (이 파일이 없으면 새 버전을 찾지 못함) |
| **설치 파일** (`CSTalk-Setup-*.exe`) | `latest.yml`의 `path`/`url`과 **실제 파일명이 일치**해야 함 |
...
1. **백엔드**가 `GET http://{백엔드}:8080/desktop-updates/latest.yml` 및 동일 베이스 URL의 설치 파일(`CSTalk-Setup-x.x.x.exe`)을 제공합니다. 파일 실제 위치는 기본값 `{APP_RELEASES_DIR}/desktop`(예: `C:\CSTalk\releases\desktop`). 다른 경로를 쓰려면 환경변수 `DESKTOP_UPDATE_DIR`을 설정합니다.
2. 새 버전 배포 시 `desktop/dist/`에서 **`latest.yml`**, **`CSTalk-Setup-{version}.exe`**, (있으면) **`.blockmap`** 을 위 `desktop` 폴더에 복사합니다. `latest.yml` 안의 `url`/`path`와 디스크의 파일명이 일치해야 합니다.
3. 사용자 PC에 **`cstalk-server.json`** 으로 `serverUrl`(예: `http://cstalk.co.kr:8080`)을 알려주면, 앱은 자동으로 `{serverUrl}/desktop-updates/` 를 업데이트 소스로 사용합니다. 파일 위치는 위 **「CSTalk 데스크톱 설치 경로」** 절을 따릅니다. 업데이트 URL만 따로 쓰려면 `updateBaseUrl`을 지정합니다.
4. `cstalk-server.json`이 없고 GitHub도 막혀 있으면 자동 업데이트는 계속 실패합니다 — 내부망에서는 반드시 `cstalk-server.json` + 위 파일 배포를 권장합니다.
...
|---|---|
| `backend/build/libs/cstalk-backend-*.jar` | `C:\CSTalk\backend\cstalk-backend.jar` |
| `realtime/` 폴더 전체 | `C:\CSTalk\realtime\` |
| `deploy/pm2.ecosystem.config.cjs` | `C:\CSTalk\realtime\pm2.ecosystem.config.cjs` |

### 3-3. 리얼타임 의존성 설치
```powershell
cd C:\CSTalk\realtime
...
$vars = @{
    ...
    "FILE_STORAGE_DIR"          = "C:/CSTalk/storage"
    "APP_RELEASES_DIR"          = "C:/CSTalk/releases"
...
> **첨부파일 경로(중요)**: 실제 저장 위치는 **DB `app_settings` 의 `file.storage.base-dir` 값이 `FILE_STORAGE_DIR` 환경변수보다 우선**합니다. 서버를 **처음** 띄울 때 환경변수가 비어 있거나 개발 기본값(`D:/testStorage`)으로 시드되면, 나중에 `FILE_STORAGE_DIR` 만 `C:/CSTalk/storage` 로 바꿔도 **DB 예전 값 때문에 업로드가 계속 실패**할 수 있습니다. 조치: 관리자 **설정** 화면에서 `file.storage.base-dir` 을 운영 경로로 수정하거나, 아래 SQL로 갱신 후 백엔드 재시작을 권장합니다. 기동 로그에 `[CSTalk] file storage ready:` 또는 `NOT writable` 이 출력됩니다.
...
```powershell
# NSSM 을 C:\tools\nssm.exe 에 저장 후
C:\tools\nssm.exe install CSTalk-Backend "java" `
  "-Xms256m -Xmx1g -jar C:\CSTalk\backend\cstalk-backend.jar"

# 서비스 시작 디렉터리 설정
C:\tools\nssm.exe set CSTalk-Backend AppDirectory "C:\CSTalk\backend"

# 로그 설정
C:\tools\nssm.exe set CSTalk-Backend AppStdout "C:\CSTalk\logs\backend-out.log"
C:\tools\nssm.exe set CSTalk-Backend AppStderr "C:\CSTalk\logs\backend-error.log"
C:\tools\nssm.exe set CSTalk-Backend AppRotateFiles 1
C:\tools\nssm.exe set CSTalk-Backend AppRotateBytes 10485760

# 서비스 시작
Start-Service CSTalk-Backend
```

서비스 상태 확인:
```powershell
Get-Service CSTalk-Backend
# Status: Running 확인
...
`C:\CSTalk\realtime\pm2.ecosystem.config.cjs` 를 열어 `DB_PASSWORD`, `REALTIME_INTERNAL_TOKEN` 등을 실제 값으로 수정합니다.
...
New-NetFirewallRule -DisplayName "CSTalk Backend 8080" -Direction Inbound `
  -Protocol TCP -LocalPort 8080 -Action Allow

# Realtime Socket.IO
New-NetFirewallRule -DisplayName "CSTalk Realtime 3001" -Direction Inbound `
...
```powershell
Add-Content -Path "C:\Windows\System32\drivers\etc\hosts" -Value "`n192.168.11.168    cstalk.co.kr"
```
...
# 브라우저 주소창: http://cstalk.co.kr:8080
```
...
생성된 `deploy\CSTalk-deploy.zip` 을 WEB 서버로 옮긴 뒤, **기존 `C:\CSTalk-deploy` 를 백업**하고 압축 해제한 다음 관리자 PowerShell에서 `.\setup-web-server.ps1` 을 다시 실행해도 되고, 이미 서비스가 있으면 **수동으로 파일만 덮어쓴 뒤 서비스만 재시작**해도 됩니다(아래 방법 B와 동일 파일 세트).

### 방법 B — 최소 패치 (이미 `C:\CSTalk` 구조가 잡혀 있을 때)

개발 PC에서 빌드 후, 아래를 WEB 서버(`192.168.11.168` 등)로 복사합니다. 경로는 설치 시 쓴 `INSTALL_DIR` 기준(문서 예시는 `C:\CSTalk`).

```powershell
# 0. 변수 (개발 PC — 저장소 루트에서 실행)
$WEB = "\\192.168.11.168\c$\CSTalk"   # 실제 서버 주소/공유로 변경

# 1. Spring Boot JAR (plain 제외한 fat JAR 하나를 cstalk-backend.jar 로 덮어쓰기)
cd backend
.\gradlew.bat bootJar
cd ..
$jar = Get-ChildItem "backend\build\libs\*.jar" | Where-Object { $_.Name -notmatch "plain" } |
       Sort-Object LastWriteTime -Descending | Select-Object -First 1
Copy-Item $jar.FullName "$WEB\backend\cstalk-backend.jar" -Force
...
#    WEB 서버에서: cd C:\CSTalk\realtime && npm install --omit=dev && pm2 restart cstalk-realtime
```

WEB 서버에서:

```powershell
Restart-Service CSTalk-Backend
# 리얼타임을 갱신했을 때만
# cd C:\CSTalk\realtime; pm2 restart cstalk-realtime
```

### 데스크톱 자동 업데이트 파일(내부망)

Electron 설치형을 배포하는 경우, 새 버전마다 `desktop` 빌드 산출물을 서버의 **`C:\CSTalk\releases\desktop`**(또는 `DESKTOP_UPDATE_DIR`)에 넣습니다.

```powershell
# 개발 PC (예: desktop\dist 에 빌드된 뒤)
$rel = "\\192.168.11.168\c$\CSTalk\releases\desktop"
Copy-Item "desktop\dist\latest.yml"           $rel -Force
Copy-Item "desktop\dist\CSTalk-Setup-*.exe"      $rel -Force
# 있으면 blockmap도 함께
```

동작 확인(브라우저 또는 WEB 서버에서):

- `http://cstalk.co.kr:8080/api/health`
- `http://cstalk.co.kr:8080/desktop-updates/latest.yml` (파일이 있으면 200·본문에 버전 정보)

환경변수 `APP_RELEASES_DIR` / `DESKTOP_UPDATE_DIR` 를 바꾼 적이 없다면 **추가 패치는 필요 없습니다**. 새 코드만 위 파일들로 반영하면 됩니다.

---

## 선택: Nginx 리버스 프록시 (포트 없이 http://cstalk.co.kr 접속)

포트 번호 없이 `http://cstalk.co.kr` 로만 접속하려면 Nginx를 추가합니다.
...
5. Windows 방화벽에서 포트 **80** 추가 개방:
   ```powershell
   New-NetFirewallRule -DisplayName "CSTalk Nginx 80" -Direction Inbound `
     -Protocol TCP -LocalPort 80 -Action Allow
   ```

Nginx 사용 시 클라이언트는 `http://cstalk.co.kr` (포트 없음) 로 접속합니다.
Socket.IO 도 Nginx가 `/socket.io/` 경로로 프록시하므로 별도 포트 불필요.

> Nginx를 사용하면 `deploy/nginx.conf` 의 `nginx.conf` 를 NSSM 으로 서비스 등록하는 것도 권장합니다.
> ```powershell
> C:\tools\nssm.exe install CSTalk-Nginx "C:\nginx\nginx.exe"
> C:\tools\nssm.exe set CSTalk-Nginx AppDirectory "C:\nginx"
> Start-Service CSTalk-Nginx
> ```
...
| 파일 업로드 실패 | DB `file.storage.base-dir` 가 존재하지 않는 드라이브·경로(예: `D:/testStorage`) | `app_settings` 또는 관리자 설정에서 `C:/CSTalk/storage` 등 실제 경로로 수정 |
| 파일 업로드 실패 | `C:\CSTalk\storage` 미생성 또는 NSSM **Local System** 에 쓰기 거부 | 폴더 생성 후 해당 폴더에 **Users** 또는 서비스 계정 **수정** 권한 부여 |
...
pgAdmin/psql 에서 (`C:/CSTalk/storage` 는 환경에 맞게 변경):

```sql
UPDATE app_settings
SET setting_value = 'C:/CSTalk/storage', updated_at = NOW()
WHERE setting_key = 'file.storage.base-dir';
```

이후 `Restart-Service CSTalk-Backend` — 기동 로그에 `[CSTalk] file storage ready: C:\CSTalk\storage` 가 보이면 정상입니다.

### UNC 공유(`\\192.168.11.179\CSTalkStorage` 등) 접근 확인
...
2. **참고 — 대화형 PowerShell만으로는 불충분**  
   ```powershell
   Test-Path \\192.168.11.179\CSTalkStorage
   "probe" | Out-File \\192.168.11.179\CSTalkStorage\_cstalk_manual_probe.txt -Encoding utf8
   Remove-Item \\192.168.11.179\CSTalkStorage\_cstalk_manual_probe.txt
   ```
