...
| 항목 | 위치 |
| :--- | :--- |
| 정책·한도·LLM HTTP | 기초설정 `ai.gateway.*`, `ai.llm.*`(시드·관리자 수정; 빈 값은 `app.ai.*`·환경 폴백) 및 `application.yml` 동일 의미 키 |
...
| 입력 코드포인트 상한 | 기초설정 `ai.gateway.llm-max-input-chars`(폴백 `app.ai.llm-max-input-chars`, 실효 256–8000). 마스킹 후 프롬프트가 초과하면 코드포인트 경계에서 잘리며 감사 detail에 `inputTruncated=true`. 상태 API `llmMaxInputChars`. |
