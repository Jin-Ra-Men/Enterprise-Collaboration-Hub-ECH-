...
### Changed
- **프론트(프로필 모달)**: `ECH화면설계 (9)` 카드형에 맞춰 `#modalUserProfile`을 히어로·부제·`.profile-dl--cards`·푸터(DM/닫기)로 정리. `openUserProfile`에서 부제에 부서·직급을 ` · `로 표시. `styles.css`에 `.ech-profile-modal` 등 보강. `DESIGN_SYSTEM.md` 매핑(9)·`FEATURE_SPEC.md`·`HANDOVER.md` 반영.
