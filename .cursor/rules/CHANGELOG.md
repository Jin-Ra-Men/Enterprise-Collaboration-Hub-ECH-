...
- **변경 요약(프론트·프레즌스 2차)**: 라이트·크림에서 온라인(선명한 초록)·오프라인(밝은 회색)·자리비움(주황) 대비 재조정. 사이드바에서 **직접 선택한 자리비움**은 포인터·키 입력 등 동작 감지로 자동 해제되지 않음(5분 무활동 자동 자리비움만 활동 시 온라인 복귀). `frontend/styles.css` · `frontend/app.js`.
...
### Changed
- **프론트(프레즌스 UI·라이트/크림)**: 오프라인 점을 어두운 슬레이트 단색 대신 **밝은 회색 채우기**·옅은 링으로 바꾸고, 온라인은 **더 선명한 초록**·자리비움은 **주황 계열** 링으로 구분. 아바타 모서리·조직도 점에도 동일 톤 적용. `frontend/styles.css`.
