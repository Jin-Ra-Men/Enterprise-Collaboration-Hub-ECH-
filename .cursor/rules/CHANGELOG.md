...
- **일정 공유 수신 UX**: `POST /api/channels/{channelId}/calendar/shares` 경로에서도 실시간 `calendar:share-request` 알림을 전송하도록 보강하고, 사이드바에 `받은 일정 공유`(`calendarShareInboxList`) 목록을 추가해 pending 공유요청을 바로 캘린더 허브로 열 수 있도록 개선.
...
### Changed
- **개인 캘린더 UI**: 워크플로(`modalWorkHub`) 내부 패널에서 분리해 전역 `#modalCalendarHub`(상단·사이드바「내 일정」). `renderCalendarHubLists`·`ensureCalendarHubBindings`·`CALENDAR_HUB_IDS`, 채널 선택 기반 공유. 문서 `FEATURE_SPEC`·`HANDOVER`·`README` 반영.
...
- **Phase 7-3 후속 (옵트인 UI·스케줄·딥링크)**: 적재 시 **채널 옵트인 필수**. `ProactiveAiSuggestionScheduler` — 매시 활동 힌트(`WORK_ITEM_HINT`, 관리자·REALTIME), 매일 09:15 Asia/Seoul 다이제스트 배치(`DIGEST_SUMMARY`, DAILY / 월요일 WEEKLY). 기초설정 `ai.proactive.activity-min-messages-per-hour`, `ai.proactive.jobs-enabled`. 제안 목록 응답 `payloadJson`, 프론트 제안함 **바로가기**, 멤버 패널 관리자 토글. `AiSuggestionKind.DIGEST_SUMMARY`, `MessageRepository.countTimelineActivitySince`. 로드맵 `7-3-3-b`.
- **Phase 7-3 MVP (프로액티브 비서·제안함)**: `channel_ai_assistant_preferences`·`user_ai_assistant_preferences`·`ai_suggestion_inbox`, REST(채널 옵트인·DM 차단·`/api/me` 톤·다이제스트·제안 목록·거절·ack), 거절 쿨다운·채널 시간당 적재 상한 기초설정(`ai.proactive.*`), 감사 `AI_ASSISTANT_*`/`AI_SUGGESTION_INBOX_*`. 프론트 환영 화면 **AI 제안함** 모달. 이관 `docs/sql/migrate_ai_phase_7_3.sql`, 테스트 `AiAssistantApiTest`. 로드맵 `7-3` 하위 항목 반영.
- **Phase 7-4-1-a**: AI 게이트웨이→LLM 전송 직전 **입력 코드포인트 상한**(`ai.gateway.llm-max-input-chars` 기초설정 시드·`app.ai.llm-max-input-chars`/`AI_GATEWAY_LLM_MAX_INPUT_CHARS` 폴백, 실효 256–8000). 초과 시 코드포인트 경계에서 잘라 전송, 성공·실패 감사 detail에 `inputTruncated=true`. `GET /api/ai/gateway/status`에 `llmMaxInputChars`. 테스트 `AiGatewayInputTruncateTest`·`AiGatewayServiceTest`. 로드맵 `7-2` 완료·`7-4-1-a` 반영.
...
### Added
- **Electron 절전 재개 복구**: `powerMonitor` `resume` → 렌더러 `ech-system-resume` IPC로 `loadMyChannels`·`recoverActiveChannelTimelineIfNeeded`·소켓 `disconnect`/`connect` 재동기화(채팅/워크플로 등 REST 고착緩和).
...
- **위치 조정 UX 안정화**: 드래그 대신 사용자 관리 표의 `▲/▼` 버튼으로 인접 행 이동을 지원. 클릭 시 현재 표시 순서를 기준으로 `directorySortOrder`를 재부여해 충돌/중복 정렬값을 방지.
